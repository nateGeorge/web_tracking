  ...
  function Evercookie(options) {
	...
	doSetOrGet("ecApplet");
      ...
      function doSetOrGet(appletId) {
        ...
        // http://stackoverflow.com/questions/16298597/call-applet-getmethod-with-javascript-throws-error-msg-typeerror-applet-get
        if (typeof(applet) != "undefined") {
            // safe to use the function
            if (value !== undefined) {
              if (typeof(applet.set) === "undefined") {
                console.log('applet.set not a function, so not using it');
              } else {
                applet.set(name,value);
              }
            }
            else {
              if (typeof(applet.get) === "undefined") {
                console.log('applet.get not a function, so not using it');
              } else {
